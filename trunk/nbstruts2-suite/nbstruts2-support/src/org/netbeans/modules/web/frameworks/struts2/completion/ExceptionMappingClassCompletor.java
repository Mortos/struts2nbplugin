...
import org.netbeans.modules.web.frameworks.struts2.completion.ui.LazyExceptionTypeItem;
 ...
 * @author Rohan Ranade
 */
public class ExceptionMappingClassCompletor extends JavaClassCompletor {

    public ExceptionMappingClassCompletor() {
    }
    ...
    protected void addNormalTypes(CompilationController cc, ClassIndex ci, 
        ...
        for (TypeElement te : tes) {
            if (JavaUtils.isExceptionType(te) && te.getQualifiedName().toString().startsWith(typedPrefix)) {
    ...
    protected void addSmartTypes(CompilationController cc, ClassIndex ci, 
        ...
        for (ElementHandle<TypeElement> eh : matchingTypes) {
            if (eh.getKind() == ElementKind.CLASS) {
                LazyTypeCompletionItem item = new LazyExceptionTypeItem(substitutionOffset, eh, cc.getJavaSource()); // NOI18N
                results.add(item);
            }
        }
    }

...
import org.netbeans.modules.framework.xwork.sp.query.ValidatorAsyncCompletionQuery;
import org.netbeans.spi.editor.completion.CompletionProvider;
import org.netbeans.spi.editor.completion.CompletionTask;
import org.netbeans.spi.editor.completion.support.AsyncCompletionTask;

/**
 * Completion provider for XWork validation XML files.
 *
 * @author Aleh Maksimovich (aleh.maksimovich@gmail.com)
 * @since 0.5.2
 */
@MimeRegistration(mimeType = XWorkMimeType.VALIDATOR_XML_MIME, service = CompletionProvider.class)
public class ValidatorCompletionProvider implements CompletionProvider {
        ...
        switch (queryType) {
            case CompletionProvider.COMPLETION_QUERY_TYPE:
                return new AsyncCompletionTask(
                        new ValidatorAsyncCompletionQuery(false), textComponent);
            case CompletionProvider.COMPLETION_ALL_QUERY_TYPE:
                return new AsyncCompletionTask(
                        new ValidatorAsyncCompletionQuery(true), textComponent);
            default:
                return null;
        }
    }
